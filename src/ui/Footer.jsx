      ...
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 48, marginBottom: 60 }}>
          ...
          <div>
            ...
            {[
              { emoji: "🥮", label: "Custom Wedding Cakes" },
              { emoji: "🎂", label: "Birthday Creations" },
              { emoji: "🧁", label: "Cupcake Collections" },
              { emoji: "🥞", label: "Seasonal Pastries" },
              { emoji: "🥧", label: "Corporate Gifting" },
              { emoji: "🍰", label: "Dessert Tables" },
            ].map((item) => (
              <div key={item.label} style={{ fontSize: 14, color: "#a07070", padding: "6px 0" }}>{item.emoji} {item.label}</div>
            ))}
          </div>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 28, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <div style={{ fontSize: 13, color: "#6a3a3a" }}>© 2026 Bebe Cakes Bakery. All rights reserved.</div>
